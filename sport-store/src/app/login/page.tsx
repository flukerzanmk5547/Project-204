...
export default function LoginPage() {
  ...
  const handleNext = () => {
    ...
    if (tab === "phone" && phone.length < 9) {
      setError("กรุณาระบุเบอร์โทรศัพท์ที่ถูกต้อง");
      return;
    }
    setError("");
    setStep("password");
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center px-4 py-8">
        <div className="w-full max-w-[480px]">
          <h1 className="text-2xl font-bold text-navy mb-6">เข้าสู่ระบบ</h1>
          ...
          {step === "identifier" ? (
            <>
              ...
              <div className="space-y-3 mt-6">
                {[
                  ...
                  <button
                    key={social.name}
                    className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-full hover:bg-gray-50 transition-colors text-sm"
                  >
                    <svg className="w-5 h-5" viewBox="0 0 24 24">
                      {social.icon}
                    </svg>
                    <span className="text-text-primary font-medium">
                      ดำเนินการต่อด้วย {social.name}
                    </span>
                  </button>
                ))}
              </div>
            ...
            >
              ...
              <button
                type="button"
                className="w-full text-sm text-blue-accent hover:underline font-medium"
              >
                ลืมรหัสผ่าน?
              </button>
            </form>
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <div className="flex items-center gap-4 text-xs text-text-secondary">
              <button className="hover:text-navy transition-colors font-medium">
                ความช่วยเหลือ
              </button>
              <button className="hover:text-navy transition-colors font-medium">
