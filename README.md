...
> เขียนชื่อในช่อง "ผู้รับผิดชอบ" หรือคอมเมนต์บอกกันได้เลย

| # | งานที่เหลือ | สถานะปัจจุบัน | ผู้รับผิดชอบ |
| --- | --- | --- | --- |
| 1 | ตั้งค่า Super Admin | รัน migration `013` แล้ว เหลือใส่ `SUPABASE_SERVICE_KEY` ใน `.env` แล้วรัน `scripts/seed-superadmin.ts` (ดูหัวข้อ "4. ตั้งค่า Super Admin") | |
| 2 | Toast แจ้งเตือนหน้าอื่น | ทำแล้วเฉพาะ `/favorites` — หน้าสินค้า / หมวดหมู่ / หน้าแรก ยังไม่มี feedback ตอนกดใส่ตะกร้า | |

### ⚠️ สิ่งที่ต้องตรวจก่อนใช้งานจริง

| เรื่อง | รายละเอียด |
| --- | --- |
| **ต้องรัน migration `014_feedbacks.sql`** | สร้างตาราง `feedbacks` — ถ้ายังไม่รัน การส่งแบบประเมินจะไม่ถูกบันทึก (หน้าเว็บยังใช้งานได้ปกติ ไม่ error ให้ผู้ใช้เห็น) |
| **RLS ของตาราง `reviews`** | module `review` ใช้ `getAdminClient()` (service role) เหมือน module อื่น ซึ่งข้าม RLS ได้ **แต่ยังไม่ได้ทดสอบกับฐานข้อมูลจริง** ถ้าตาราง `reviews` เปิด RLS ไว้ ให้ลองเขียนรีวิวจริงดูว่าบันทึกผ่านไหม |
| **ทดสอบ Rate Limit** | ตั้งไว้ 300 req/นาที/IP — หน้าแรกยิงหลาย endpoint พร้อมกัน ถ้าเจอ 429 ระหว่างใช้งานปกติให้เพิ่มค่า `RATE_LIMIT_MAX` ใน `.env` |
| **`is_verified` ของรีวิว** | ตอนนี้ยังเป็น `false` เสมอ ถ้าอยากให้ขึ้นป้าย "ซื้อจริง" ต้องเช็คว่า user เคยสั่งซื้อสินค้านั้นจริงก่อนตั้งค่าเป็น `true` |
| **ค่าเฉลี่ยรีวิวของ seed เดิม** | `products.rating` / `review_count` ที่มาจาก seed จะถูกเขียนทับด้วยค่าจริงเมื่อมีคนรีวิวสินค้านั้นครั้งแรก |

### สรุปงานที่เพิ่งเสร็จ

| งาน | รายละเอียด |
| --- | --- |
| 🐛 แก้บั๊ก role | `008_roles.sql` ไม่มี `superadmin` ใน CHECK แต่โค้ดใช้อยู่ (`AuthPlugin`, `AuthSchema`, `seed-superadmin.ts`) → ทำให้ตั้ง superadmin ไม่ได้ แก้ใน `sql/013_superadmin_role.sql` |
...
| ✅ เอกสาร / diagram | sync ตาม role ใหม่ (customer / reseller / manager / superadmin) ครบทุกไฟล์ |
