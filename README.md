...
> **สถานะ:** ✅ ใช้งานได้จริง (frontend ต่อ backend แล้ว) · 🟡 ใช้งานได้แต่ยังไม่บันทึกลงฐานข้อมูล · ❌ ยังไม่ได้ทำ
>
> ตรวจสอบสถานะจากโค้ดจริงเมื่ออัปเดตล่าสุด
...
| 1.5 | แถบประกาศ (Announcement bar) | ✅ | `GET /api/v1/homepage/config/announcement_text` (มีข้อความสำรองถ้า API ล่ม) |
...
| 3.4 | รีวิวสินค้า + คะแนน | ✅ | `/api/v1/reviews` — เขียน/ลบรีวิว, สรุปดาว, อัปเดตค่าเฉลี่ยกลับที่สินค้า |

### 4. ตะกร้าสินค้า

| # | ฟีเจอร์ | สถานะ | หมายเหตุ |
| --- | --- | --- | --- |
| 4.1 | เพิ่ม / แก้ไขจำนวน / ลบสินค้า | ✅ | `/api/v1/cart` เมื่อล็อกอิน · localStorage เมื่อเป็น guest |
| 4.2 | แผงตะกร้าด้านข้าง (CartPanel) | ✅ | |
| 4.3 | คำนวณยอดรวม / ส่วนลด / ค่าส่ง | ✅ | ยอดจาก server เมื่อล็อกอิน · คำนวณฝั่ง client เมื่อเป็น guest |
| 4.4 | เชื่อมตะกร้ากับบัญชีผู้ใช้ | ✅ | ล็อกอินแล้ว merge ตะกร้า guest ขึ้น server และล้าง localStorage ทิ้ง |
...
| # | ฟีเจอร์ | สถานะ | หมายเหตุ |
| --- | --- | --- | --- |
| 6.1 | หน้า checkout หลายขั้นตอน | ✅ | ที่อยู่ → จัดส่ง → ชำระเงิน ต่อ API ครบ (`getAddresses`, `createOrder`, `createPayment`) |
...
| # | ฟีเจอร์ | สถานะ | หมายเหตุ |
| --- | --- | --- | --- |
| 7.1 | กดหัวใจเพิ่ม/เอาออก | ✅ | sync กับ API เมื่อล็อกอิน · localStorage เมื่อยังไม่ล็อกอิน |
| 7.4 | Toast แจ้งเตือนตอนใส่ตะกร้า | ✅ | หน้า `/favorites` |
| 7.2 | หน้ารวมสินค้าโปรด `/favorites` | ✅ | |
| 7.3 | ผูกสินค้าโปรดกับบัญชีผู้ใช้ | ✅ | `/api/v1/favorites` + ตาราง `user_favorites` (merge ของเดิมตอนล็อกอิน, ล้างตอน logout) |

### 8. แบบประเมินความพึงพอใจ (Feedback)

| # | ฟีเจอร์ | สถานะ | หมายเหตุ |
| --- | --- | --- | --- |
| 8.1 | แผงประเมิน (คะแนน / วัตถุประสงค์) | ✅ | ส่งได้ทั้งผู้ใช้ทั่วไปและสมาชิก (ถ้าล็อกอินจะผูกกับบัญชีให้) |
| 8.2 | บันทึกผลประเมิน | ✅ | `POST /api/v1/feedbacks` + ตาราง `feedbacks` (migration `014`) |
| 8.3 | ดูผลประเมิน (reseller ขึ้นไป) | ✅ | `GET /api/v1/feedbacks` และ `/summary` (ค่าเฉลี่ย, แยกคะแนน, แยกวัตถุประสงค์) |
...
| 9.4 | **หน้าจอ Backoffice** | ✅ | `/backoffice` 15 หน้า (สินค้า/คำสั่งซื้อ/ผู้ใช้/แบนเนอร์/analytics ฯลฯ) |
...
| --- | --- | --- | --- |
| 10.1 | Module ที่มีจริง (20) | ✅ | auth, product, category, cart, banner, homepage, promotion, bundle, attribute, recommendation, address, order, payment, favorite, notification, dashboard, analytics, superadmin, review, feedback |
...
| 10.6 | Helmet / Rate Limit | ✅ | `SecurityPlugin` + `RateLimitPlugin` (300 req/นาที/IP, ยกเว้น `/api/health`) |
| 10.7 | ฐานข้อมูล 31 ตาราง | ✅ | migration `001` – `014` |
| 10.8 | ตาราง `user_addresses` / `payments` / `user_favorites` / `notifications` | ✅ | migration 003–013 |

## 📢 แจ้งทีม (จากพลช)

อัปเดตล่าสุด — push ขึ้น `main` หมดแล้ว และ **ตรวจกับ DB จริงเรียบร้อย** พร้อมให้ทุกคนมาเทสได้เลย

### ✅ ทำอะไรไปแล้วบ้าง (รอบนี้)
1. **ระบบรีวิวสินค้า** — module `review` + หน้าสินค้าแสดงรีวิวจริง เขียน/ลบได้ มีกราฟแยกดาว และอัปเดตคะแนนเฉลี่ยกลับไปที่สินค้าอัตโนมัติ
2. **ระบบแบบประเมิน (Feedback)** — เดิมกดส่งแล้วข้อมูลหาย ตอนนี้เก็บลง DB แล้ว + ดูผลสรุปได้ (reseller ขึ้นไป)
3. **เปิดใช้ Helmet + Rate Limit** (300 request/นาที/IP)
4. **Toast แจ้งเตือน** ตอนกดใส่ตะกร้าในหน้าสินค้าโปรด
5. เอกสาร/diagram อัปเดตให้ตรง role ใหม่ (customer / reseller / manager / superadmin) ครบทุกไฟล์

### 🐛 บั๊กที่เจอและแก้แล้ว (3 อัน)
1. **superadmin ชน constraint** — `008_roles.sql` อนุญาต role แค่ customer/reseller/manager แต่โค้ดใช้ `superadmin` ด้วย → ตั้ง superadmin ไม่ได้ **แก้ใน `sql/013_superadmin_role.sql`**
2. **ตะกร้าลบแล้วเด้งกลับ** — ลบสินค้าแล้ว refresh/เพิ่มใหม่ ของที่ลบกลับมา เกิดจาก race condition (ลบสินค้าที่เพิ่งเพิ่มก่อน API commit → ไม่ได้ยิง DELETE จริง) **แก้โดยทำ mutation ตะกร้าเป็นคิวเรียงลำดับใน `CartContext`**
3. **สินค้าซ้ำ ไม่แยกหมวดหมู่** — หมวดย่อยที่ไม่มีสินค้าเด้งไปโชว์ของหมวดแม่ ทำให้หลายหมวดโชว์ชุดเดียวกัน **แก้โดยเอา fallback หมวดแม่ออกจาก `CategoryService`** ให้แต่ละหมวดโชว์เฉพาะ subtree ของตัวเอง

### ✅ ตรวจกับ Supabase จริงแล้ว — DB พร้อมหมด
- migration `013` + `014` **รันแล้ว**
- Super Admin **seed แล้ว** (superadmin 1 · manager 1 · customer 5)
- ระบบ Feedback ทดสอบส่ง → บันทึกลง DB ได้จริง (end-to-end)
- บั๊กหมวดหมู่ **ยืนยันหาย** (ยิง endpoint จริง หมวดว่างคืน 0 ไม่เด้งไปดึงของแม่)

→ **ไม่ต้องรัน migration อะไรเพิ่มแล้ว** ใครจะเทสแค่ตั้ง `.env` + `.env.local` แล้ว `npm run dev` 2 ฝั่ง (ดูหัวข้อ "วิธีติดตั้งและรันโปรเจกต์")

### 🟡 งานที่เหลือ — ใครสนใจจองได้ (เขียนชื่อในตารางหัวข้อ "🚧 สถานะงานปัจจุบัน")
- Toast หน้าอื่น (ตอนนี้มีเฉพาะหน้าสินค้าโปรด)
- ป้าย "ซื้อจริง" ในรีวิว (ต้องเช็คประวัติสั่งซื้อก่อน)
- หน้าดูผลแบบประเมินใน backoffice (API พร้อมแล้ว ขาดแค่หน้าจอ)

### ⚠️ ฝากเทสยืนยันหน่อย
- บั๊กตะกร้า: ล็อกอิน → เพิ่มสินค้า → ลบ → refresh ต้องไม่กลับมา (แก้ที่ฝั่งเบราว์เซอร์ อยากให้ลองกดจริง)
- เขียนรีวิว/ส่งแบบประเมินจริง เผื่อติด RLS
- ถ้าเจอ error 429 ระหว่างใช้งานปกติ บอกด้วย เดี๋ยวปรับ Rate Limit ให้

**สถานะตอนนี้:** ฟีเจอร์เสร็จ **49 รายการ** · บั๊กที่รู้ทั้งหมดแก้แล้ว · เอกสาร/diagram ครบ
รายละเอียดทั้งหมดดูหัวข้อ "🚧 สถานะงานปัจจุบัน" ด้านล่าง

---

## 🚧 สถานะงานปัจจุบัน

> บันทึกไว้ให้ทีมรู้ว่าทำถึงไหนแล้ว — เขียนชื่อในช่อง "ผู้รับผิดชอบ" ได้เลยถ้าจะรับงานไหนต่อ

**ภาพรวมฟีเจอร์:** ทำเสร็จแล้ว **49 รายการ** (ดูตารางหัวข้อ "ฟีเจอร์หลัก" ด้านบน) — ตรวจสอบสถานะจากโค้ดจริงทุกข้อ

### ✅ ตรวจสอบกับ DB จริงแล้ว — พร้อมให้เทส

ตรวจกับ Supabase production เมื่อ 22 ก.ค. 2026:

| รายการ | สถานะ |
| --- | --- |
| migration `013` (superadmin role) | ✅ รันแล้ว |
| migration `014` (ตาราง feedbacks) | ✅ รันแล้ว |
| Super Admin seed | ✅ มีแล้ว (superadmin 1 · manager 1 · customer 5) |
| ระบบ Feedback | ✅ ทดสอบส่ง → บันทึกลง DB ได้ (end-to-end) |
| 🐛 บั๊กหมวดหมู่ซ้ำ | ✅ **ยืนยันหาย** — ยิง endpoint จริง หมวดว่าง (`sports/outdoor/fishing`) คืน 0 ไม่เด้งไปดึงของหมวดแม่ แต่ละหมวดได้สินค้าของตัวเอง |
| 🐛 บั๊กตะกร้าเด้งกลับ | ✅ แก้แล้ว (client-side `CartContext`) — ฝากเทสในเบราว์เซอร์: เพิ่มสินค้า → ลบ → refresh ต้องไม่กลับมา |

> **DB ตั้งค่าครบแล้ว** ผู้ที่จะเทสแค่สร้าง `.env` (ฝั่ง api) + `.env.local` (ฝั่ง frontend) แล้ว `npm run dev` ทั้ง 2 ฝั่ง (ดูหัวข้อ "วิธีติดตั้งและรันโปรเจกต์")

### 🟡 งานที่เหลือ (ไม่บล็อกการใช้งาน)

| # | งาน | รายละเอียด | ผู้รับผิดชอบ |
| --- | --- | --- | --- |
| 1 | Toast แจ้งเตือนหน้าอื่น | ทำแล้วเฉพาะ `/favorites` — หน้าสินค้า / หมวดหมู่ / หน้าแรก ยังไม่มี feedback ตอนกดใส่ตะกร้า | |
| 2 | ป้าย "ซื้อจริง" ในรีวิว | `is_verified` ยังเป็น `false` เสมอ ต้องเช็คว่าผู้ใช้เคยสั่งซื้อสินค้านั้นก่อนตั้งเป็น `true` | |
| 3 | หน้าดูผลแบบประเมินใน backoffice | API พร้อมแล้ว (`GET /api/v1/feedbacks` + `/summary`) แต่ยังไม่มีหน้าจอแสดงผล | |

### ⚠️ ยังไม่ได้ทดสอบกับฐานข้อมูลจริง

| เรื่อง | รายละเอียด |
| --- | --- |
| **RLS ของตาราง `reviews` / `feedbacks`** | ทั้งสอง module ใช้ `getAdminClient()` (service role) เหมือน module อื่น ซึ่งข้าม RLS ได้ **แต่ยังไม่ได้ทดสอบเขียนจริง** ตอนเทสให้ลองเขียนรีวิวและส่งแบบประเมินดูว่าบันทึกผ่านไหม |
| **Rate Limit** | ตั้งไว้ 300 req/นาที/IP — หน้าแรกยิงหลาย endpoint พร้อมกัน ถ้าเจอ error 429 ระหว่างใช้งานปกติให้เพิ่ม `RATE_LIMIT_MAX` ใน `.env` |
| **ค่าเฉลี่ยรีวิวจาก seed** | `products.rating` / `review_count` ที่มาจาก seed จะถูกเขียนทับด้วยค่าจริงเมื่อมีคนรีวิวสินค้านั้นครั้งแรก |

### ✅ งานที่ทำเสร็จแล้ว (รอบล่าสุด)
...
| ✅ **Toast หน้า favorites** | แจ้งเตือนมุมขวาล่างตอนกดใส่ตะกร้า |
| ✅ **เอกสาร / diagram** | sync ตาม role ใหม่ (customer / reseller / manager / superadmin) ครบทุกไฟล์ + ตรวจสถานะฟีเจอร์ใหม่ทั้งหมดจากโค้ดจริง |
...
### 4. ตั้งค่า Super Admin (ทำครั้งเดียว)

> ✅ **รัน migration `013_superadmin_role.sql` ใน Supabase SQL Editor แล้ว** (เพิ่ม role `superadmin` เข้า constraint)
> ขั้นตอนด้านล่างคือส่วนที่ **ยังต้องทำต่อ**

**4.1 เพิ่ม Service Role Key ใน `.env`**

สคริปต์ seed ต้องใช้ `SUPABASE_SERVICE_KEY` (คนละตัวกับ `SUPABASE_ANON_KEY` และยังไม่มีใน `.env.example`)

เอามาจาก: Supabase → **Project Settings → API Keys → `service_role`**

```env
SUPABASE_SERVICE_KEY=<service_role key>
```

> ⚠️ key นี้ข้ามสิทธิ์ทุกอย่างในฐานข้อมูล **ห้าม commit ขึ้น git** ใส่ใน `.env` เท่านั้น

**4.2 รันสคริปต์สร้างบัญชี**
...
บัญชีที่ได้ — **ควรเปลี่ยนรหัสผ่านหลังใช้งานจริง**

| Email | Password |
| --- | --- |
| `superadmin@sportgear.com` | `SuperAdmin@2024!` |

**4.3 ตรวจสอบ**

```sql
-- รันใน Supabase SQL Editor
SELECT email, role FROM profiles WHERE role <> 'customer';
```

แล้วลองเข้า `http://localhost:3000/backoffice/login` ด้วยบัญชีข้างบน ต้องเห็นเมนูจัดการผู้ใช้/พนักงานได้
