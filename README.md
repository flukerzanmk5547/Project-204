...
### 🔴 ต้องทำก่อนใช้งาน (สำคัญ — ยังไม่ได้ทำ)

| # | สิ่งที่ต้องทำ | ทำไมต้องทำ | ผู้รับผิดชอบ |
| --- | --- | --- | --- |
| 1 | **รัน `sql/014_feedbacks.sql`** ใน Supabase SQL Editor | สร้างตาราง `feedbacks` — ถ้ายังไม่รัน แบบประเมินจะส่งแล้วไม่ถูกบันทึก (หน้าเว็บยังใช้ได้ปกติ ไม่ error) | |
| 2 | **ตั้งค่า Super Admin** | รัน migration `013` แล้ว เหลือใส่ `SUPABASE_SERVICE_KEY` ใน `.env` แล้วรัน `scripts/seed-superadmin.ts` (ดูหัวข้อ "4. ตั้งค่า Super Admin") ถ้ายังไม่ทำ จะเข้า backoffice ระดับ superadmin ไม่ได้ | |
