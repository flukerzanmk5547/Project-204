...
> อัปเดตล่าสุด — งานค้างเดิมเคลียร์ไปแล้วเกือบหมด เหลือ 2 ข้อ

| # | งานที่เหลือ | สถานะปัจจุบัน | ผู้รับผิดชอบ |
| --- | --- | --- | --- |
| 1 | ตั้งค่า Super Admin | รัน migration `013` แล้ว เหลือใส่ `SUPABASE_SERVICE_KEY` ใน `.env` แล้วรัน `scripts/seed-superadmin.ts` (ดูขั้นตอนหัวข้อ "4. ตั้งค่า Super Admin") | |
| 2 | ระบบ Feedback | เป็น UI อย่างเดียว ยังไม่ส่งข้อมูลไป backend และไม่มีตารางเก็บ | |
| 3 | Toast แจ้งเตือนหน้าอื่น | ตอนนี้ทำเฉพาะหน้า `/favorites` — หน้าสินค้า/หมวดหมู่/หน้าแรก ยังไม่มี feedback ตอนกดใส่ตะกร้า | |

**สรุปงานที่เพิ่งเสร็จ**

| งาน | รายละเอียด |
| --- | --- |
| 🐛 แก้บั๊ก role | `008_roles.sql` ไม่มี `superadmin` ใน CHECK แต่โค้ดใช้อยู่ → เพิ่มใน `sql/013_superadmin_role.sql` |
| ✅ ระบบรีวิว | module `review` ครบ (schema/repository/service/controller/routes) + หน้าสินค้าแสดงรีวิวจริง เขียน/ลบได้ มีกราฟแยกดาว และอัปเดตค่าเฉลี่ยกลับไปที่ตาราง `products` อัตโนมัติ |
| ✅ Helmet + Rate Limit | เปิดใช้แล้วผ่าน `SecurityPlugin` / `RateLimitPlugin` |
