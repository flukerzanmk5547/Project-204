...
push งานขึ้น `main` แล้วนะครับ สรุปให้ตามนี้
...
2. **ระบบแบบประเมิน (Feedback)** — เดิมกดส่งแล้วข้อมูลหายเลย ตอนนี้เก็บลงฐานข้อมูลแล้ว และดูผลสรุปได้ (reseller ขึ้นไป)
3. **เปิดใช้ Helmet + Rate Limit** (300 request/นาที/IP)
4. **Toast แจ้งเตือน** ตอนกดใส่ตะกร้าในหน้าสินค้าโปรด

**🐛 เจอบั๊กและแก้แล้ว**
`008_roles.sql` กำหนด role ได้แค่ `customer` / `reseller` / `manager` แต่โค้ด (`AuthPlugin`, `AuthSchema`, `seed-superadmin.ts`) ใช้ `superadmin` ด้วย ทำให้ตั้ง superadmin ไม่ได้เพราะชน constraint — แก้ไว้ใน `sql/013_superadmin_role.sql` แล้ว

**🔴 ฝากช่วยกันทำก่อนใช้งาน (สำคัญ)**

ถ้าไม่ทำ 2 ข้อนี้ เวลาเทสจะเจอว่า superadmin ล็อกอินไม่ได้ และแบบประเมินไม่ถูกบันทึก ซึ่งดูเหมือนโค้ดพัง ทั้งที่แค่ยังไม่ได้รัน SQL

1. **รัน migration ใน Supabase SQL Editor 2 ไฟล์**
   - `sql/013_superadmin_role.sql`
   - `sql/014_feedbacks.sql`
2. **ตั้งค่า Super Admin** (ใครสะดวกทำได้เลย) — เอา `service_role` key จาก Supabase → Project Settings → API Keys แล้วใส่เป็น `SUPABASE_SERVICE_KEY` ใน `.env` ของ `sport-store-api` จากนั้นรัน `npx tsx scripts/seed-superadmin.ts`
   > ⚠️ key นี้ข้ามสิทธิ์ทุกอย่างในฐานข้อมูล **ห้าม commit ขึ้น git**

**🟡 งานที่เหลือ — ใครสนใจจองได้ เขียนชื่อในตารางด้านล่างได้เลย**
...
**⚠️ ฝากเทสให้หน่อย (ผมยังไม่ได้ลองกับฐานข้อมูลจริง)**
- ลองเขียนรีวิวและส่งแบบประเมินดูว่าบันทึกผ่านไหม เผื่อติด RLS
- ถ้าเจอ error 429 ระหว่างใช้งานปกติ บอกด้วย เดี๋ยวปรับ Rate Limit ให้

**สถานะตอนนี้:** ฟีเจอร์เสร็จ **49 รายการ** · เอกสารและ diagram (Use Case / Class / Sequence / Architecture / Deployment) อัปเดตครบแล้ว
